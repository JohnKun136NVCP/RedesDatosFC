...
char *server_ip = SERVER_IP, *alias;
bool  my_turn = false, active_turn = false, free_turn=true;	// Para los turnos
...
char* next_alias() {
    if(strcmp(alias, "s01") == 0) return "s02";
    if(strcmp(alias, "s02") == 0) return "s03";
    if(strcmp(alias, "s03") == 0) return "s04";
    if(strcmp(alias, "s04") == 0) return "s01";
	else "";
}

void escribe_alias(char *a) {
	FILE *fp;
	fp = fopen("turn.txt", "wb");
	if (fp == NULL) {
		perror("[-] Error to open the file");
		return;
	}
...
int manejar_conexiones(int server_sock, int port) {
	...
	printf("[%s][%s] Waiting for my turn...\n", alias, get_time());
	// Para los turnos
	time_t now, start;
	...
	while(true) {
		...
		if(!my_turn) {
			...
			fclose(fp);
			// printf("[%s][%s] \n", alias, file_content);
		    if(strcmp(alias, file_content) == 0) my_turn = true;
		}
		// Activar turno
		if(my_turn && !active_turn) {
			start = time(NULL);
			active_turn = true;
			printf("[%s][%s] Turn started...\n", alias, get_time());
		}
		// Renovar turno
		if(active_turn) {
			now = time(NULL);
			// Terminar turno por falta de peticiones
			if((now - start >= QUANTUM/5) && free_turn) {
				my_turn = active_turn = false;
				printf("[%s][%s] Turn ended...\n", alias, get_time());
				// Escribe el archivo
				escribe_alias(next_alias());
			}
			
			// Terminar turno por tiempo
			if(now - start >= QUANTUM) {
				my_turn = active_turn = false;
				printf("[%s][%s] Turn ended...\n", alias, get_time());
				// Escribe el archivo
				escribe_alias(next_alias());
        ...
        if(FD_ISSET(server_sock, &ready_sockets) && active_turn) {
...
int main(int argc, char *argv[]) {
    ...
    if(strcmp(argv[1], "s03") == 0) server_ip = S03_IP;
    if(strcmp(argv[1], "s04") == 0) server_ip = S04_IP;
	alias = argv[1];
    if(server_ip == SERVER_IP) {
        printf("Invalid name of server: %s \n", argv[1]);
        return 1;
    }
	// Crear archivo para determinar los turnos
	FILE *fp = fopen("turn.txt", "r");
	if (fp == NULL) 
		escribe_alias(alias);
	else 
		fclose(fp);
