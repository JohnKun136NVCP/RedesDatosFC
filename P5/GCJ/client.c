...
void *send_file_thread(void *argumentos){
    ...
    int conexion = -1; 
    while (conexion == -1 || conexion == 1){
        conexion = connect_to_server(argumentos_struct->puerto, argumentos_struct->alias);
        if (conexion == -1 || conexion == 1){
            printf("[-] Retry: Could not connect to %s:%d\n", argumentos_struct->alias, argumentos_struct->puerto);
            sleep(1);
        }   
    }
