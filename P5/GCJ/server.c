...
sem_t resource_sem; // Semáforo utilizado para controlar el tiempo de ejecución.
...
/** Limpiamos los recursos antes de salir. */
void cleanup() {
...
int create_sock_server(const char *ip, int puerto){
    ...
    snprintf(mensaje_escucha, sizeof(mensaje_escucha), "Server listening on port %d\n", puerto);
...
void *recieve_file_thread(void *arg){
    ...
    // Continuamos "formando" el proceso mientras no reciba nada
    while (true){
        printf("[- %s] Waiting on semaphore\n", argumentos->interfaz); 
        sem_wait(&resource_sem);
        printf("[- %s] Acquired semaphore, waiting on client...\n", argumentos->interfaz);
        struct sockaddr_in client_addr_logistico;
        socklen_t addr_size = sizeof(client_addr_logistico);
        fd_set set;
        struct timeval timeout;
        FD_ZERO(&set);
        FD_SET(server_sock_logistico, &set);
        timeout.tv_sec = 2;  
        timeout.tv_usec = 0;
        int rv = select(server_sock_logistico + 1, &set, NULL, NULL, &timeout);
        if(rv == -1){
            printf("[-] Error on selection\n");
            close(server_sock_logistico);
            pthread_exit(NULL);
        } else if (rv == 0){
            printf("[* %s] No client connected, passing turn\n", argumentos->interfaz);
        } else {
            int client_sock_logistico = accept(server_sock_logistico, (struct sockaddr *)&client_addr_logistico, &addr_size); //Aceptamos la conexión del cliente. 
            printf("[* %s] Client connected, holding semaphore\n", argumentos ->interfaz);
            if(client_sock_logistico < 0){
                ...
                int client_sock_comunicacion = accept(server_sock_comunicacion, (struct sockaddr *)&client_addr_comunicacion, &addr_size); //Aceptamos la conexión del cliente por el puerto de datos.
                ...
                close(server_sock_comunicacion);
                sem_post(&resource_sem); //Soltamos el semáforo antes de salir de la ejecución.
                pthread_exit(NULL);
            }
        }
        sem_post(&resource_sem); //Soltamos el semáforo para otro proceso.
        usleep(100000);
    }
}

// Nos permite revisar si nuestra lista de argumentos contiene duplicados.
bool contiene_duplicados(char *arr[], int elementos){
...
int main(int argc, char *argv[])
{
    ...
    int cant_servidores = argc - 1;
    char *interfaces[cant_servidores];
    char *ip[cant_servidores];
    int puerto = 4200;
    // Extraemos los argumentos del programa.
    for (int i = 0; i < cant_servidores; i++){
        char *interfaz = argv[i+1];
        interfaces[i] = interfaz; 
        // Configuración de IPs, se puede modificar.
        if (strcmp(interfaz, "s01") == 0) {
            ip[i] = "192.168.201.101"; 
        } else if (strcmp(interfaz, "s02") == 0) {
            ip[i] = "192.168.201.102";
        } else if (strcmp(interfaz, "s03") == 0) {
            ip[i] = "192.168.201.104";
        } else if (strcmp(interfaz, "s04") == 0){
            ip[i] = "192.168.201.105";
        }
        else {
            printf("Type: <INTERFAZ_IP>\n");
            return 1;
    ...
    if(contiene_duplicados(interfaces, sizeof(interfaces) / sizeof(interfaces[0]))){
        printf("[-] Only one of each interface can be created.");
        return 1;
    }
    pthread_t hilos[cant_servidores]; 
    struct argumentos_servidor argumentos[cant_servidores];
    for (int i = 0; i < cant_servidores; i++){
        argumentos[i].puerto = puerto;
        argumentos[i].ip = ip[i]; 
        argumentos[i].interfaz = interfaces[i];
        argumentos[i].i = i+1;
    }
