...
void *recieve_file_thread(void *arg){
    ...
    if (argumentos->puerto != argumentos->puerto_valido){
        printf("[*][SERVER %d] Request rejected (client requested port %d) \n", argumentos->puerto_valido, argumentos->puerto);
        close(server_sock);
        pthread_exit(NULL);
    }
...
int main(int argc, char *argv[])
{
    ...
    pthread_t hilos[3]; 
    struct argumentos_servidor argumentos[3] = {
        {"mensaje.txt", puerto + 2, puerto},
        {"mensaje.txt", puerto + 1, puerto},
        {"mensaje.txt", puerto, puerto},
    };
    for (int i = 0; i < 3; i++){
