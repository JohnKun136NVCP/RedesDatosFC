...
int main(int argc, char *argv[])
{
    ...
    {
        printf("Usage: %s <SERVIDOR_IP> <PUERTOS...> <ARCHIVOS...> <DESPLAZAMIENTO>\n", argv[0]);
        return 1;
    }
    ...
    int n = (argc - 3) / 2;
    int *puertos = malloc(n * sizeof(int));
    char **archivos = malloc(n * sizeof(char*));
    ...
    for (int i = 0; i < n; i++) {
        archivos[i] = argv[2 + n + i];
        if (check_file(archivos[i]) == 0) {
    ...
    struct argumentos_envio *argumentos = malloc(n * sizeof(struct argumentos_envio));
    ...
    for (int i = 0; i < n; i++) {
        argumentos[i].ip = ip;
        argumentos[i].puerto = puertos[i];
        argumentos[i].filename = archivos[i];
