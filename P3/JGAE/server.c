...
int main() {
    ...
    */
    ...
    buffer[bytes] = '\0';
    // "String scan formatted", escaneamos la cadena con formato, pasamos el buffer que es de donde va a sacar los datos
    // "%s %d" indica que va a sacar una cadena (%s) seguida de un espacio y luego un número entero (%d)
    // ambos los guardamos en clave y shift
    sscanf(buffer, "%s %d", clave, &shift); // extrae clave y desplazamiento
    // usamos &shift para pasar la dirección de la variable, para clave no usamos porque al ser string ya es un apuntador
    ...
    if (isOnFile(clave)) {
        ...
        // Generamos la información y se la mandamos
        saveNetworkInfo("network_info.txt");
        sendFile("network_info.txt", client_sock);
        printf("[+] Sent file\n");
	// Generación del archivo de información del sistema y también se lo mandamos
