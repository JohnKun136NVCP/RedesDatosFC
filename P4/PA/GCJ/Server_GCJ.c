...
void recieve_encrypt_buffer(char *archivo, int client_sock, int server_sock){
    ...
    char shift_recieved[] = "Shift and filenmae recieved";
...
int switch_port(int puerto_actual, int client_sock){
    ...
    int puerto_actual_nombre = ntohs(puerto_actual_sock.sin_port); //Obtenemos el nombre del puerto que estamos escuchando.
    int puerto_siguiente_nombre = puerto_actual_nombre + 1; //Aumentamos en 1 para el puerto de comunicación.
    char puerto_siguiente_str[42]; 
    snprintf(puerto_siguiente_str, sizeof(puerto_siguiente_str), "%d", puerto_siguiente_nombre);
    send(client_sock, puerto_siguiente_str, strlen(puerto_siguiente_str), 0); //Enviamos el nuevo puerto.
    close(puerto_actual); //Cerramos la comunicación por el puerto inicial.
    close(client_sock);
    char mensaje_cerrado[100]; 
    snprintf(mensaje_cerrado, sizeof(mensaje_cerrado), "Port %d closed for comunication", puerto_actual_nombre); 
    logger(mensaje_cerrado, strlen(mensaje_cerrado));
    int puerto_siguiente = create_sock_server(puerto_siguiente_nombre); //Abrimos la comunicación por el puerto de datos.
    char mensaje_abierto[100]; 
    snprintf(mensaje_abierto, sizeof(mensaje_abierto),"Port %d created for file exchange", puerto_siguiente_nombre);
...
void *recieve_file_thread(void *arg){
    ...
    int server_sock_logistico = create_sock_server(argumentos->puerto); //Creamos el nuevo socket en base a los argumentos.
    ...
    int server_sock_comunicacion = switch_port(server_sock_logistico, client_sock_logistico); //Realizamos el cambio de puerto al puerto de datos.
    ...
    recieve_encrypt_buffer(archivo_a_recibir, client_sock_comunicacion, server_sock_comunicacion);
