...
static void handle_data(int cfd, int listen_port, const char *alias) {
    char line[LINE_MAX];
    ...
    if (!body) return;
    if (read_n(cfd, body, len) != 0) { free(body); return; }
    body[len] = '\0';

    if (porto == listen_port) {
        encryptCaesar(body, shift);

        // Crear directorio ~/s01 o ~/s02
        char dir[512], path[600];
        snprintf(dir, sizeof(dir), "%s/%s", home_dir(), alias);
        mkdir(dir, 0755);

        snprintf(path, sizeof(path), "%s/prueba_Red.txt", dir);
        ...
        (void)send_all(cfd, "PROCESADO\n", 11);
        printf("[SERVER] Guardado en %s\n", path);
    } else {
        (void)send_all(cfd, "RECHAZADO\n", 10);
    }
...
int main(int argc, char **argv) {
    ...
    int ctrl_port = 49200;
    int data_port = 49201;  // puerto superior
    ...
    for (;;) {
        ...
        if (FD_ISSET(ls_ctrl, &r)) {
            int cfd = accept(ls_ctrl, NULL, NULL);
            if (cfd >= 0) {
                char out[64];
        ...
        if (FD_ISSET(ls_data, &r)) {
            int cfd = accept(ls_data, NULL, NULL);
            if (cfd >= 0) {
                handle_data(cfd, data_port, alias);
                close(cfd);
            }
        }
    }
