...
void saveSystemInfo(const char *outputFile) {
    ...
    fpOutput = fopen(outputFile, "w");
    if (fpOutput == NULL) {
        perror("[-] No se pudo abrir sysinfo.txt");
        return;
    }

    // OS y Kernel
    fprintf(fpOutput, "OS y Kernel\n");
    fpCommand = popen("uname -a", "r");
    if (fpCommand) {
        while (fgets(buffer, sizeof(buffer), fpCommand)) {
            fputs(buffer, fpOutput);
        }
        pclose(fpCommand);
    }
    ...
    // Distribución
    fprintf(fpOutput, "\nDistribución\n");
    fpCommand = popen("cat /etc/os-release", "r");
    if (fpCommand) {
        while (fgets(buffer, sizeof(buffer), fpCommand)) {
            fputs(buffer, fpOutput);
        }
    ...
    // CPU
    fprintf(fpOutput, "\nCPU\n");
    fpCommand = popen("lscpu | head -n 5", "r");
    if (fpCommand) {
        while (fgets(buffer, sizeof(buffer), fpCommand)) {
            fputs(buffer, fpOutput);
        }
    ...
    fclose(fpOutput);
    printf("[+] Archivo sysinfo.txt generado correctamente\n");
}

// Envía un archivo al cliente en bloques
...
// Verifica si la clave existe en cipherworlds.txt (ignorando mayúsculas y espacios)
...
int main() {
    ...
    if (isOnFile(clave)) {
        ...
        // Guardamos la infor del sistema, no la mandamos pero podríamos
