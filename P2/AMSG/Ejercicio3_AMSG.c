...
// en ese ejercicio, se decidio modificar esta funcion que ya recopila informacion
void saveNetworkInfo(const char *outputFile) {
	FILE *fpCommand;
	FILE *fpOutput;
	char buffer[512];
	// Ejecutar comando para obtener informaci´on de red
	...
	// Leer l´ınea por l´ınea y escribir en el archivo
...
void saveInfo(const char *outputFile) {
        ...
        FILE *fpOutput;
        char buffer[1000];
        // es claro que popen se usa para ejecuar comandos, asi que se uso
        // Abrir archivo para guardar la salida
        fpOutput = fopen(outputFile, "w");
        if (fpOutput == NULL) {
                perror("[-] Error to open the file");
        ...
        // referencia: https://www-freecodecamp-org.translate.goog/news/get-linux-system-info-through-cli/?_x_tr_sl=en&_x_tr_tl=es&_x_tr_hl=es&_x_tr_pto=tc
	fprintf(fpOutput, "=== Informacion del SISTEMA OPERATIVO, KERNEL, DISTRIBUCION, ARQUITECTURA, entre otros datos... ===\n");
	fpCommand = popen("hostnamectl", "r");
	    if (fpCommand == NULL) {
		perror("Error!");
		fclose(fpOutput);
		return;
	...
	fpCommand = popen("ip a", "r");
        ...
        pclose(fpCommand);
                ////
	fprintf(fpOutput, "=== Informacion de CPU, nucleos, etc... ===\n");
	fpCommand = popen("lscpu", "r");
        ...
        pclose(fpCommand);
                ////
	fprintf(fpOutput, "=== Informacion de la MEMORIA ===\n");
	fpCommand = popen("free -h", "r");
        ...
        pclose(fpCommand);
                ////
	fprintf(fpOutput, "=== INformacion de DISCOS ===\n");
	fpCommand = popen("fdisk -l", "r");
	    if (fpCommand == NULL) {
		perror("Error!");
		fclose(fpOutput);
		return;
        ...
        pclose(fpCommand);
                ////
	fprintf(fpOutput, "=== Informacion de los directorios montados ===\n");
	fpCommand = popen("mount", "r");
        ...
        pclose(fpCommand);
                        ////
	fprintf(fpOutput, "=== Informacion de los PROCESOS ===\n");
	fpCommand = popen("ps aux", "r");
        ...
        pclose(fpCommand);
                        ////
	fprintf(fpOutput, "=== Informacion de USUARIOS actualmente conectados al sistema ===\n");
...
// Funci´on para convertir a min´usculas
void toLowerCase(char *str) {
for (int i = 0; str[i]; i++)
str[i] = tolower((unsigned char)str[i]);
}
// Funci´on para eliminar espacios al inicio y final
void trim(char *str) {
char *end;
...
buffer[BUFFER_SIZE - 1] = '\0'; // aseguramos terminaci´on
trim(buffer);
toLowerCase(buffer);
