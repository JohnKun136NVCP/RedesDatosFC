...
// Función de descifrado César inverso
...
// Guardar información de red en archivo
...
// Guardar información del sistema en sysinfo.txt
void saveSystemInfo(const char *outputFile) {
    FILE *fp = fopen(outputFile, "w");
    if (fp == NULL) {
        perror("[-] Error opening sysinfo.txt");
        return;
    }
    ...
    fprintf(fp, "\n[+] Distribución:\n");
    system("cat /etc/os-release | grep PRETTY_NAME >> sysinfo.txt");
    ...
    fprintf(fp, "\n[+] Procesos activos:\n");
    system("ps aux --no-heading | wc -l >> sysinfo.txt");

    fprintf(fp, "\n[+] Directorios montados:\n");
    system("mount | column -t >> sysinfo.txt");
...
// Validar clave con archivo cipherworlds.txt
...
int main() {
    ...
    if (isOnFile(clave)) {
        ...
        // Enviar archivo sysinfo.txt al cliente
