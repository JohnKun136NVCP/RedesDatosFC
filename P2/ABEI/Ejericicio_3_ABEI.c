...
void saveSystemInfo(const char *outputFile) {
    ...
    fpOutput = fopen(outputFile, "w");
    if (fpOutput == NULL) {
        perror("[-] Error opening sysinfo file");
        return;
    }

    const char *commands[] = {
        "echo '=== OS y Kernel ===' && uname -a",
        "echo '\n=== Distribución ===' && lsb_release -a 2>/dev/null || cat /etc/*release",
        "echo '\n=== IPs ===' && ip addr show",
        "echo '\n=== CPU ===' && lscpu | grep -E 'Model name|CPU\\(s\\)|Architecture'",
        "echo '\n=== Memoria ===' && free -h",
        "echo '\n=== Disco ===' && df -h",
    ...
    for (int i = 0; i < numCommands; i++) {
        ...
        if (fpCommand == NULL) {
            fprintf(fpOutput, "[-] Error executing command: %s\n", commands[i]);
            continue;
        }
...
// Verifica la clave en el archivo
