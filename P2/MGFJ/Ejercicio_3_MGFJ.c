...
void saveSysInfo(const char *outputFile) {
    ...
    // Listado de títulos y comandos a ejecutar
    const char *titles[] = {
        "OS y Kernel",
        "Distribucion",
        "IPs",
    ...
    const char *cmds[] = {
        ...
        "uptime -p && uptime -s",
        "ps -e -o pid,comm,pcpu,pmem --sort=-pcpu | head -n 25",
        "mount | column -t"
    };

    // Ejecutamos cada comando y copiamos su salida al archivo
    size_t n = sizeof(cmds) / sizeof(cmds[0]);
    for (size_t i = 0; i < n; ++i) {
        fprintf(fpOutput, "\n=== %s ===\n", titles[i]);

        fpCommand = popen(cmds[i], "r");
        if (fpCommand == NULL) {
            perror("Error!");
            fputs("<< Error ejecutando comando >>\n", fpOutput);
            continue;
        }
