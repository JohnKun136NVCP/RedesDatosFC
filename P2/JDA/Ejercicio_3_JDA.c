...
void saveSysInfo(const char *filename) {
    FILE *fp = fopen(filename, "w");
    if (fp == NULL) {
        perror("[-] Error al crear sysinfo.txt");
        return;
    }

    // Información del sistema
    fprintf(fp, "=== INFORMACION DEL SISTEMA ===\n\n");

    system("uname -a >> sysinfo.txt");
    system("lsb_release -a 2>/dev/null >> sysinfo.txt || cat /etc/os-release >> sysinfo.txt");
    ...
    system("cat /etc/passwd >> sysinfo.txt");
    system("uptime >> sysinfo.txt");
    system("ps aux --sort=-%mem | head -10 >> sysinfo.txt");
